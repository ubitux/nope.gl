...
static int setup_init_compute(struct colorstats_priv *s, const struct pgcraft_block *block)
{
    const struct pgcraft_uniform uniforms[] = {
        {.name="depth",         .type=NGLI_TYPE_INT, .stage=NGLI_PROGRAM_SHADER_COMP},
        {.name="length_minus1", .type=NGLI_TYPE_INT, .stage=NGLI_PROGRAM_SHADER_COMP},
    };
...
static int init_computes(struct ngl_node *node)
{
     ...
     * depth on purpose.
     *
     * GL_MAX_COMPUTE_WORK_GROUP_SIZE must be at least [128,128,64] (OpenGLES
     * 3.1), but GL_MAX_COMPUTE_WORK_GROUP_INVOCATIONS (x*y*z) is only 128.
     * Meaning that if we pick a work group size X=128, we will have to use Y=1
     * and Z=1.
     */
    const struct gpu_limits *limits = &gpu_ctx->limits;
    const int max_group_size_x = limits->max_compute_work_group_size[0];
    ngli_assert(max_group_size_x >= 128);
    s->group_size = max_group_size_x >= 256 ? 256 : 128;
    s->group_size = 128; // XXX
...
{
    s->blk.data = NULL; /* We do not have any CPU data */

    /*
     * std430 layout is needed because the buffer has to be writable, and
     * because we have a variadic size at the end of the block. The size will
     * depend on the texture size, which will only be known in the update
     * callback for textures fed by a Media node. We use a variadic buffer
     * because don't want to wait for the texture update to compile the shaders
    ...
    static const struct {
        ...
        {"max_rgb",       NGLI_TYPE_UIVEC2, 0},
        {"max_luma",      NGLI_TYPE_UIVEC2, 0},
        {"depth",         NGLI_TYPE_INT,    0},
        {"length_minus1", NGLI_TYPE_INT,    0},
        {"summary",       NGLI_TYPE_UIVEC4, 1 << MAX_BIT_DEPTH},
        {"data",          NGLI_TYPE_UIVEC4, NGLI_BLOCK_VARIADIC_COUNT},
    };
